...
{	 
	...
	int n_of_bytes=sizeof(revision_number), retval;
	retval = ec_SDOread(m_slave_nr,  0x1018, 0x03, FALSE, &n_of_bytes , &revision_number, EC_TIMEOUTSAFE);
	if (0==retval || sizeof(revision_number)!=n_of_bytes){
		Logger::In in(this->getName());
		log(Error)<< m_datap->name<<" : Problem reading revision_number value from slave. Values: retval: "<<retval
	...
	std::cout<<"Application version: "<< std::hex<<least_significant_bit<<std::endl;
	...
	n_of_bytes=sizeof(motor_rated_torque_unit);
...
void SoemMaxPos::update()
{
	...
	//updates ports only if the using the cyclic modes.
	//otherwise the use operation only to write.
	if (current_mode_of_operation==8 ||current_mode_of_operation==9 ||
			current_mode_of_operation==10){
